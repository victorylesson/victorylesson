<!-- HEADER -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=292D32&height=200&section=header&text=Victory%20Mendes&fontSize=50&fontColor=F2F2F2&animation=fadeIn&fontAlignY=35&desc=Desenvolvedor%20Full%20Stack%20%7C%20Node.js%20%7C%20Python%20%7C%20MongoDB%20%7C%20React&descAlignY=55&descSize=18"/>
</p>

<p align="center">
  <a href="mailto:victory.mendes@hotmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/victory-lesson/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://victory-dev.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>
  <a href="https://wa.me/5581991482982">
    <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"/>
  </a>
</p>

---

##  Sobre Mim

Desenvolvedor Full Stack focado em desenvolvimento web, APIs, automação de dados e aplicações escaláveis.

Atualmente cursando Sistemas de Informação na UNINASSAU.

 Recife, PE — Brasil

Tenho experiência com desenvolvimento de aplicações usando Node.js, Python, React e MongoDB, além de deploy e integração de serviços em nuvem.

Disponível para freelances, projetos e parcerias.

---

##  Tecnologias

### Linguagens

<p>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
</p>

### Backend

<p>
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white"/>
  <img src="https://img.shields.io/badge/Express-000000?style=for-the-badge&logo=express&logoColor=white"/>
  <img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white"/>
  <img src="https://img.shields.io/badge/REST_API-FF6C37?style=for-the-badge&logo=postman&logoColor=white"/>
</p>

### Frontend

<p>
  <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black"/>
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white"/>
  <img src="https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white"/>
</p>

### Banco de Dados

<p>
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white"/>
  <img src="https://img.shields.io/badge/Mongoose-880000?style=for-the-badge&logo=mongoose&logoColor=white"/>
</p>

### Dados e Automação

<p>
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
  <img src="https://img.shields.io/badge/OpenPyXL-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white"/>
  <img src="https://img.shields.io/badge/Jinja-B41717?style=for-the-badge&logo=jinja&logoColor=white"/>
</p>

### Deploy e Ferramentas

<p>
  <img src="https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  <img src="https://img.shields.io/badge/Render-46E3B7?style=for-the-badge&logo=render&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white"/>
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white"/>
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white"/>
  <img src="https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white"/>
</p>

##  Projetos

###  Portfolio

<p>
  <a href="https://github.com/victorylesson/portfolio-victory-dev">
    <img src="https://img.shields.io/badge/Código-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://victory-dev.vercel.app/">
    <img src="https://img.shields.io/badge/Live-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>
</p>

Portfolio pessoal desenvolvido com Node.js e MongoDB Atlas.

**Tecnologias:** Node.js · MongoDB · Vercel

---

###  Auto Relatório

<p>
  <a href="https://github.com/victorylesson/auto-relatorio">
    <img src="https://img.shields.io/badge/Código-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://auto-relatorio-three.vercel.app/">
    <img src="https://img.shields.io/badge/Live-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>
</p>

Ferramenta para leitura de arquivos `.xlsx` e `.csv` e geração de relatórios em HTML e JSON.

**Tecnologias:** Python · Pandas · OpenPyXL · Jinja · Vercel

---

##  Serviços

| Serviço | Descrição | Tecnologias |
|---|---|---|
|  Websites | Sites, landing pages e portfólios | React · HTML · CSS · Vercel |
|  APIs | APIs REST, autenticação e CRUD | Node.js · Express · MongoDB · JWT |
|  Banco de Dados | Modelagem e organização de dados | MongoDB · Mongoose |
|  Automação | Processamento e relatórios | Python · Pandas · OpenPyXL |
|  Deploy | Hospedagem e CI/CD | Vercel · Render · GitHub Actions |
|  Manutenção | Correções e evolução de projetos | Stack completo |

---

##  Processo

**1. Descoberta**

Entendimento do projeto e definição das necessidades.

**2. Proposta**

Escopo, prazo e investimento.

**3. Desenvolvimento**

Implementação e acompanhamento do projeto.

**4. Entrega**

Deploy, documentação e orientação.

**5. Suporte**

Correções e manutenção após a entrega.

---

##  Contato

Estou disponível para freelances, projetos e parcerias.

<p align="center">
  <a href="mailto:victory.mendes@hotmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://wa.me/5581991482982">
    <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"/>
  </a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=292D32&height=100&section=footer"/>
</p>
